...
#+freelancer start
  #  WEEKLY PAY AND CHECKS FOR UPGRADE
    ...
    (24 * 7, [
		...
		(try_begin),
		   ...
		   (else_try),
				...
				(else_try),
					...
					(display_message, "@Your arms are to weak to advance in archery at this moment."),
				(else_try),
					(eq, ":reason", 3), #not enough riding skill for horse
					(display_message, "@You require more horse riding skills to fit your next position!"),
				(try_end),				
		   (try_end),			
        (try_end),
		...
		#gekokujo 3.0 different pay levels for ashigaru and samurai start
		...
		(try_begin),
			...
			(store_mul, ":weekly_pay", 20, ":level"), #samurai get paid more
		(else_try),
			(store_mul, ":weekly_pay", 10, ":level"), #ashigaru get paid the same
		(try_end),
		#gekokujo 3.0 different pay levels for ashigaru and samurai end
		
		
		#gekokujo 3.0 monks obviously do not get paid start
		#(troop_add_gold, "trp_player", ":weekly_pay"),
		#(add_xp_to_troop, 70, "trp_player"),
		#(play_sound, "snd_money_received", 0),
		(add_xp_to_troop, 70, "trp_player"),
		(try_begin),
			(neg|is_between, "$player_cur_troop", monk_troops_begin, monk_troops_end),
			(troop_add_gold, "trp_player", ":weekly_pay"),
			(play_sound, "snd_money_received", 0),
		(try_end),
		#gekokujo 3.0 monks obviously do not get paid end
		
		(val_add, "$time_in_service", 1), #gekokujo 3.0 time in service counter is based on paydays
    ]),
