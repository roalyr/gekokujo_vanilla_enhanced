...
# ADDS THE PLAYER TO THE LORD'S PARTY  
    ...
    [
		...
		(store_mul, ":samurai_potential", reg0, 5),
		...
		(else_try),
			...
			(faction_get_slot, "$player_cur_troop", ":commander_faction", slot_faction_tier_0_troop),
		(else_try),
		...
		#gekokujo 3.0 remember the player's original banner, in case they were noble start
		(troop_get_slot, "$freelancer_original_banner", "trp_player", slot_troop_banner_scene_prop),
...
#  RUNS IF THE PLAYER LEAVES THE ARMY
   ...
   ("event_player_discharge",
    [
		...
		#gekokujo 3.0 with 10 or more relations, you can keep your equipment start
		(call_script, "script_troop_get_player_relation", "$enlisted_lord"),
		(try_begin),
			(ge, reg0, 10),
			(display_message, "@You have been allowed to keep your equipment!"), 
		(else_try),
			(call_script, "script_freelancer_unequip_troop", "$player_cur_troop"),		
			(troop_equip_items, "trp_player"),
		(try_end),
		#gekokujo 3.0 with 10 or more relations, you can keep your equipment end

		(troop_set_slot, "trp_player", slot_troop_current_mission, 0),
		(troop_set_slot, "trp_player", slot_troop_days_on_mission, 0),
		#gekokujo 3.0 remember player's original banner, in case they were noble
		...
		#gekokujo 3.0 finish quest abuse fix start
		#(call_script, "script_finish_quest", "qst_freelancer_enlisted", 100), #percentage--make based on days served?
		(call_script, "script_finish_quest", "qst_freelancer_enlisted", 75), #75 ensures that there is no relation change
		#gekokujo 3.0 finish quest abuse fix end
    ]),
...
# RUNS WHEN PLAYER RETURNS FROM VACATION
  ...
  ("event_player_returns_vacation",
    [
		...
		#gekokujo 3.0 what the hell this looks like it was missing
		(assign, "$freelancer_state", 1),
		
		#gekokujo remove the vacation mission gracefully
		(call_script, "script_end_quest", "qst_freelancer_vacation"),
    ]),

#gekokujo 3.0 runs when returning from defeat start
  ("event_player_returns_defeat",
    [
        (troop_set_slot, "trp_player", slot_troop_current_mission, 0),
		(troop_set_slot, "trp_player", slot_troop_days_on_mission, 0),
		...
		#gekokujo remove the vacation mission gracefully
		(call_script, "script_end_quest", "qst_freelancer_captured"),
    ]),
#gekokujo 3.0 runs when returning from defeat end
  ...
  ("event_player_deserts",
   [     
	...
	#gekokujo 3.0 remember player's original banner, in case they were noble $freelancer_original_banner
	#(troop_set_slot, "trp_player", slot_troop_banner_scene_prop, 0),
	(troop_set_slot, "trp_player", slot_troop_banner_scene_prop, "$freelancer_original_banner"),
	(rest_for_hours, 0,0,0),
	(assign, "$freelancer_state", 0),
...
#gekokujo 3.0 player decides not to return after defeat start
  ("event_player_retires_defeat",
   [     
   	(store_troop_faction, ":commander_faction", "$enlisted_lord"),
	(troop_set_slot, "trp_player", slot_troop_current_mission, 0),
	(troop_set_slot, "trp_player", slot_troop_days_on_mission, 0),
...
#gekokujo 3.0 player decides not to return after defeat end
	...
	#gekokujo 3.0 freelancer dismiss companions at enlistment start
	#party restore is iffy in freelancer so we just dismiss the companions normally
	#you will now be able to find them via travellers or going to their forts
    ("dismiss_companions", 
    [
		(party_get_num_companion_stacks, ":num_stacks", "p_main_party"),
		(try_for_range, ":cur_stack", 0, ":num_stacks"),
			(party_stack_get_troop_id, ":dismiss_troop", "p_main_party", ":cur_stack"),
			(neq, ":dismiss_troop", "trp_player"), #isn't the player
			(troop_is_hero, ":dismiss_troop"), #is a hero
			
			(troop_set_slot, ":dismiss_troop", slot_troop_occupation, 0),
			(troop_set_slot, ":dismiss_troop", slot_troop_playerparty_history, pp_history_dismissed),
			(str_store_troop_name, s11, ":dismiss_troop"),
			(try_begin), 
				#we do something extra for fort companions
				(is_between, ":dismiss_troop", fort_companions_begin, fort_companions_end), #is a fort companion
				(troop_get_slot, ":home", ":dismiss_troop", slot_troop_home),
				(party_set_slot, ":home", slot_fort_npc_2_state, 2), #set to "return"
				(display_message, "@{s11} has left for home."),
			(else_try),
				#is a regular companion
				(display_message, "@{s11} has left to find greener pastures."), 
			(try_end),
			(remove_member_from_party, ":dismiss_troop"),
		(try_end),
    ]),
	#gekokujo 3.0 freelancer dismiss companions at enlistment end
   ...
   [
	...
	(try_for_range, ":type", itp_type_horse, itp_type_bullets),
		...
		(else_try),
			...
			#gekokujo 2.1a make freelancer recognize guns
		...
		(else_try),
		    ...
		    #(lt, ":open_weapon_slot", 4), #gekokujo 3.0 freelancer sashimono (explained below)
		    (lt, ":open_weapon_slot", 3),
		...
		(try_begin),
			#gekokujo 2.1a argh goddamn fucking bullets
		    #(is_between, ":type", itp_type_one_handed_wpn, itp_type_head_armor), #Uses one of the 4 weapon slots
			(is_between, ":type", itp_type_one_handed_wpn, itp_type_bullets), #Uses one of the 4 weapon slots
			#gekokujo 2.1a make sure it isn't a piece of armor
			...
			#gekokujo 2.1a give players bullets with their guns start
            (else_try),
				(this_or_next|eq, ":type", itp_type_pistol),
				(eq, ":type", itp_type_musket),
				(assign, ":needs_ammo", itp_type_bullets),
			#gekokujo 2.1a give players bullets with their guns end
			(try_end),
		(try_end),
	(try_end), #Item Types Loop
	
	#gekokujo 3.0 freelancer sashimono start
	(assign, ":item", -1),
	(store_character_level, ":troop_level", ":source_troop"),
	(store_troop_faction, ":faction", "$enlisted_lord"),
	(store_sub, ":offset", ":faction", "fac_kingdom_1"),
	
	(try_begin), #samurai
		(is_between, ":source_troop", samurai_troops_begin, samurai_troops_end),
		(try_begin), 
			(is_between, ":troop_level", 6, 17), #samurai retainers
			(store_add, ":item", "itm_gekokujo_sashimono_uesugi_2", ":offset"),
		(else_try), 
			(eq, ":troop_level", 21), #samurai officers
			(neg|troop_is_guarantee_ranged, ":source_troop"), #no master archers or gunners
			(try_begin), #there is no elegant way of doing this
				...
				(assign,":item","itm_gekokujo_sashimono_3"),
			(else_try),
				...
				(assign,":item","itm_gekokujo_sashimono_4"),
			(else_try),
				#date, asakura, hojo, mogami, ryuzoji, and ukita
				(assign,":item","itm_gekokujo_sashimono_5"),
			(try_end),
		(try_end), 
	(else_try), #ashigaru
		(is_between, ":source_troop", ashigaru_troops_begin, ashigaru_troops_end),
		(try_begin), #among trained ashigaru, only spearmen get sashimono
			(eq, ":troop_level", 14),
			(neg|troop_is_guarantee_ranged, ":source_troop"),
			(store_add, ":item", "itm_gekokujo_sashimono_uesugi_1", ":offset"),
		(else_try), #among veteran ashigaru and above, all get sashimono
			(ge, ":troop_level", 19),
			(store_add, ":item", "itm_gekokujo_sashimono_uesugi_1", ":offset"),
		(try_end),
	(try_end),
	...
	(try_begin), 
		...
		(str_store_string, s2, "@{s3}, {s2}"),
	(try_end),
	#gekokujo 3.0 freelancer sashimono end
	
    #add ammo for any equipped bow
    (try_begin),
	    (neq, ":needs_ammo", 0),		
		#Check for item already in the last slot, move it if present
		#gekokujo 3.0 for the sake of potential sashimono, move ammo to third slot
		(try_begin), 
			#(troop_get_inventory_slot, ":old_item", "trp_player", ek_item_3), #gekokujo 3.0 freelancer sashimono
			(troop_get_inventory_slot, ":old_item", "trp_player", ek_item_2), #gekokujo 3.0 freelancer sashimono
			(gt, ":old_item", 0),
			#(troop_get_inventory_slot_modifier, ":old_item_imod", "trp_player", ek_item_3), #gekokujo 3.0 freelancer sashimono
			(troop_get_inventory_slot_modifier, ":old_item_imod", "trp_player", ek_item_2), #gekokujo 3.0 freelancer sashimono
			(troop_add_item, "trp_player", ":old_item", ":old_item_imod"), 
		(try_end),
		...
		(try_for_range, ":inv_slot", 0, ":end"),
			...
			(eq, ":type", ":needs_ammo"),
			#(troop_set_inventory_slot, "trp_player", ek_item_3, ":item"), #gekokujo 3.0 freelancer sashimono
			(troop_set_inventory_slot, "trp_player", ek_item_2, ":item"), #gekokujo 3.0 freelancer sashimono
			(party_set_slot, "p_freelancer_party_backup", ":recording_slot", ":item"),
		    (val_add, ":recording_slot", 1),
			#(assign, ":open_weapon_slot", 4), #gekokujo 3.0 freelancer sashimono
			(assign, ":open_weapon_slot", 3), #gekokujo 3.0 freelancer sashimono
			(str_store_item_name, s3, ":item"),
		    (str_store_string, s2, "@{s3}, {s2}"),
			(assign, ":end", 0),
		(try_end),
	...
	(try_begin),
		...
		(try_for_range, ":inv_slot", 0, ":end"),
			...
			(try_begin),
			    ...
			    (gt, ":open_weapon_slot", 2),
			    (assign, ":open_weapon_slot", 1),
				#gekokujo 3.0 freelancer sashimono end
			(try_end),
